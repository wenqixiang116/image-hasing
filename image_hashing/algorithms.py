...
def marr_hildreth_hash(image, hash_size=8, scale=4, alpha=2.5):
    ...
    # Generate final hash by subsampling and detecting zero-crossings
    # Since we are subsampling, detecting "zero crossings" by checking if > 0
    # provides the binary blocks.
    diff_subsampled = log_image[::scale, ::scale] > 0

    return _binary_array_to_hex(diff_subsampled.flatten())
