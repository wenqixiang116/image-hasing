...
def dhash_vertical(image, hash_size=8):
    ...
    # Compute differences vertically
    pixels = np.asarray(image)
    diff = pixels[1:, :] > pixels[:-1, :]
...
def marr_hildreth_hash(image, hash_size=8, scale=4):
    ...
    # Convert to float and apply Laplacian of Gaussian
    pixels = np.asarray(image, dtype=float)
    blocks = scipy.ndimage.gaussian_laplace(pixels, sigma=2.5)

    # Subsample back to hash_size x hash_size
    subsampled = blocks[0::scale, 0::scale]

    # Find zero crossings (using < 0 as specified)
    diff = subsampled < 0
