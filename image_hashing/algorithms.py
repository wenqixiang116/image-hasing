...
def dhash_vertical(image, hash_size=8):
    ...
    # Compute differences
    pixels = np.asarray(image)
    # Compare pixel[x, y] to pixel[x, y+1] (which is rows, so pixels[1:, :] > pixels[:-1, :])
    diff = pixels[1:, :] > pixels[:-1, :]

    return _binary_array_to_hex(diff.flatten())

def marr_hildreth_hash(image, hash_size=8, alpha=2.5, scale=2):
    """
