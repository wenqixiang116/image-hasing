...
def marr_hildreth_hash(image, hash_size=16, alpha=2.5, scale=4):
    """
    ...
    log_image = scipy.ndimage.gaussian_laplace(pixels, alpha)

    # Subsample
    subsampled = log_image[::scale, ::scale]

    # Compute bits (zero-crossings)
    diff = subsampled > 0
