...
def marr_hildreth_hash(image, hash_size=8, alpha=2.5, scale=4):
    """
    Marr-Hildreth Hash computation (edge based).
    """
    image_scale = hash_size * scale
    image = image.resize((image_scale, image_scale), Image.Resampling.LANCZOS)
    image = image.convert("L")
    pixels = np.asarray(image)

    # Apply LoG
    log = scipy.ndimage.gaussian_laplace(pixels, sigma=alpha)

    # Sample
    log_sampled = log[::scale, ::scale]
    log_sampled = log_sampled[:hash_size, :hash_size]
