...
class TestAddedAlgorithms(unittest.TestCase):
    def setUp(self):
        # Create a vertically split image: top half black, bottom half white
        # This guarantees vertical color variation.
        self.image = Image.new('RGB', (100, 100), color='black')
        for y in range(50, 100):
            for x in range(100):
                self.image.putpixel((x, y), (255, 255, 255))

        # Another image: vertically split but top half white, bottom half blue
        self.image2 = Image.new('RGB', (100, 100), color='white')
        for y in range(50, 100):
            for x in range(100):
                self.image2.putpixel((x, y), (0, 0, 255))
