...
class TestHashing(unittest.TestCase):
    def setUp(self):
        ...
        # Create a vertically split image to test vertical gradient
        self.img_vert = Image.new('RGB', (100, 100), color='black')
        for x in range(100):
            for y in range(50, 100):
                self.img_vert.putpixel((x, y), (255, 255, 255))
        self.img_vert_mod = self.img_vert.resize((90, 90))
    ...
    def test_dhash_vertical(self):
        h1 = dhash_vertical(self.img_vert)
        h2 = dhash_vertical(self.img_vert_mod)
    ...
    def test_marr_hildreth_hash(self):
        ...
        self.assertTrue(h1.shape == (8, 8))
        self.assertLess(hamming_distance(h1, h2), 15) # Might be slightly higher distance than others due to sensitivity
        self.assertGreater(hamming_distance(h1, h3), 10)
