...
class TestHashing(unittest.TestCase):
    ...
    def test_dhash_vertical(self):
        ...
        # dhash_vertical checks differences vertically. Let's ensure it handles split correctly
        split_img = Image.new('RGB', (100, 100), color='white')
        d = ImageDraw.Draw(split_img)
        d.rectangle([0, 50, 100, 100], fill='black')
        h_split = dhash_vertical(split_img)

        # The result shouldn't be all zero
    ...
    def test_marr_hildreth_hash(self):
        ...
        self.assertTrue(h1.shape == (8, 8))
        self.assertLess(hamming_distance(h1, h2), 15) # marr hildreth could be a bit sensitive to size changes depending on scale
        self.assertGreater(hamming_distance(h1, h3), 10)
