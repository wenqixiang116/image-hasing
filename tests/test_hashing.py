...
class TestHashing(unittest.TestCase):
    ...
    def test_dhash_vertical(self):
        # Create an image that is split vertically
        vert_img = Image.new('RGB', (100, 100), color='white')
        d = ImageDraw.Draw(vert_img)
        d.rectangle([0, 50, 100, 100], fill='black')

        h = dhash_vertical(vert_img)
        self.assertTrue(h.shape == (8, 8))
        # Ensure it creates a meaningful hash (non-zero array)
        self.assertTrue(np.any(h))

        # Test consistency with resized image
        h_resized = dhash_vertical(vert_img.resize((90, 90)))
        self.assertLess(hamming_distance(h, h_resized), 5)

        # Test with a different image
        h_diff = dhash_vertical(self.img3)
        self.assertGreater(hamming_distance(h, h_diff), 5)
    ...
    def test_marr_hildreth_hash(self):
        ...
        self.assertTrue(h1.shape == (8, 8))
        # Check consistency for slightly changed image
        self.assertLess(hamming_distance(h1, h2), 5)
        # Compare hashes of different images to ensure they are not equal
