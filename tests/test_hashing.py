...
class TestHashing(unittest.TestCase):
    ...
    def test_dhash_vertical(self):
        img_v_split = Image.new('RGB', (100, 100), color='white')
        d = ImageDraw.Draw(img_v_split)
        d.rectangle([0, 50, 100, 100], fill='black')

        h1 = dhash_vertical(img_v_split)
        self.assertTrue(h1.shape == (8, 8))
        # Verify it's not all zeros or all ones
        self.assertTrue(0 < np.count_nonzero(h1) < 64)
    ...
    def test_marr_hildreth_hash(self):
        ...
        self.assertTrue(h1.shape == (8, 8))
        self.assertLess(hamming_distance(h1, h2), 15) # Might be slightly more sensitive to resizing
