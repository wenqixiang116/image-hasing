...
class TestHashing(unittest.TestCase):
    ...
    def test_dhash_vertical(self):
        # Memory says: "When testing dhash_vertical, test images must have vertical color variation
        # (e.g., be vertically split) to avoid generating an all-zero hash..."
        img1 = Image.new('RGB', (100, 100), color='white')
        d1 = ImageDraw.Draw(img1)
        d1.rectangle([0, 50, 100, 100], fill='black') # Vertically split

        img2 = img1.resize((90, 90)) # Same content, different size

        img3 = Image.new('RGB', (100, 100), color='black') # Different content

        h1 = dhash_vertical(img1)
        h2 = dhash_vertical(img2)
        h3 = dhash_vertical(img3)
