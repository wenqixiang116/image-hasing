...
class TestNewAlgorithms(unittest.TestCase):
    def setUp(self):
        ...
        # Image with horizontal split (vertical variation)
        self.image_hsplit = Image.new('RGB', (100, 100), color='black')
        for y in range(50, 100):
            for x in range(100):
                self.image_hsplit.putpixel((x, y), (255, 255, 255))

    def test_dhash_vertical(self):
        h = dhash_vertical(self.image_hsplit)
        self.assertIsInstance(h, str)
        self.assertEqual(len(h), 16)

        h2 = dhash_vertical(self.image_hsplit)
