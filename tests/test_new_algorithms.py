...
from image_hashing import whash, colorhash, marr_hildreth_hash
...
class TestNewAlgorithms(unittest.TestCase):
    ...
    def test_marr_hildreth_hash(self):
        h = marr_hildreth_hash(self.image, scale=2)
        self.assertIsInstance(h, str)
        self.assertEqual(len(h), 16) # 8x8 = 64 bits = 16 hex chars

        h2 = marr_hildreth_hash(self.image, scale=2)
        self.assertEqual(h, h2)

        # Test diff
        h3 = marr_hildreth_hash(self.image2, scale=2)
        self.assertNotEqual(h, h3)
