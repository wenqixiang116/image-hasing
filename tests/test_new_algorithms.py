...
class TestNewAlgorithms(unittest.TestCase):
    ...
    def test_dhash_vertical(self):
        # We need an image with vertical variation, otherwise
        # horizontal split results in all zeros.
        from PIL import ImageDraw
        img_h_split = Image.new('RGB', (100, 100), color='black')
        d = ImageDraw.Draw(img_h_split)
        d.rectangle([0, 50, 100, 100], fill='white')

        h = dhash_vertical(img_h_split)
        self.assertIsInstance(h, str)
        self.assertEqual(len(h), 16)

        h2 = dhash_vertical(img_h_split)
        self.assertEqual(h, h2)
