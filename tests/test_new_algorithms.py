...
class TestNewAlgorithms(unittest.TestCase):
    def setUp(self):
        ...
        # Create a simple image: black on top, white on bottom (for vertical dhash)
        self.v_image = Image.new('RGB', (100, 100), color='black')
        for x in range(100):
            for y in range(50, 100):
                self.v_image.putpixel((x, y), (255, 255, 255))
    ...
    def test_dhash_vertical(self):
        h = dhash_vertical(self.v_image)
        self.assertIsInstance(h, str)
        self.assertEqual(len(h), 16) # 8x8 = 64 bits = 16 hex chars

        h2 = dhash_vertical(self.v_image)
        self.assertEqual(h, h2)

        # Ensure we're not generating an all-zero hash due to lack of vertical variation
