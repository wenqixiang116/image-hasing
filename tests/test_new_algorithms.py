...
class TestNewAlgorithms(unittest.TestCase):
    def setUp(self):
        ...
        # Vertically split image (top black, bottom white)
        self.image_vsplit = Image.new('RGB', (100, 100), color='black')
        for x in range(100):
            for y in range(50, 100):
                self.image_vsplit.putpixel((x, y), (255, 255, 255))
    ...
    def test_dhash_vertical(self):
        h = dhash_vertical(self.image_vsplit)
        self.assertIsInstance(h, str)
        self.assertEqual(len(h), 16)

        h2 = dhash_vertical(self.image_vsplit)
        self.assertEqual(h, h2)
