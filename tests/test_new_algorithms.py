...
class TestNewAlgorithms(unittest.TestCase):
    def setUp(self):
        ...
        self.vsplit_image = Image.new('RGB', (100, 100), color='white')
        d = ImageDraw.Draw(self.vsplit_image)
    ...
    def test_dhash_vertical(self):
        h = dhash_vertical(self.vsplit_image)
        self.assertIsInstance(h, str)
        self.assertEqual(len(h), 16) # 8x8 = 64 bits = 16 hex chars

        h2 = dhash_vertical(self.vsplit_image)
        self.assertEqual(h, h2)
